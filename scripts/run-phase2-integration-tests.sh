...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      ...
      echo "  GIT_REPO_URL           Repository URL (read from .env.local if available)"
      ...
      echo "  GIT_TOKEN              Git token for private HTTPS repos (read from .env.local if available)"
      echo "  GIT_SSH_KEY            Path to SSH key for private SSH repos"
      echo "  AWS_PROFILE            AWS profile name"
      echo "  AWS_REGION             AWS region"
      echo ""
      echo "Note: GIT_REPO_URL and GIT_TOKEN can be set in .env.local for convenience"
      echo ""
      echo "Examples:"
