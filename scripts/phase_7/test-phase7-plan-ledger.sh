#!/bin/bash
# Phase 7 E2E: Plan Ledger — seed one PAUSED plan, POST resume, GET /plans/:planId/ledger, assert entries; cleanup.
...
if [ -z "$REGION" ] || [ -z "$PLANS_TABLE" ]; then
  echo "Phase 7 E2E (plan ledger) requires: AWS_REGION, REVENUE_PLANS_TABLE_NAME (or REVENUE_PLANS_TABLE). Set in .env or environment."
  exit 1
fi
...
aws lambda invoke --region "$REGION" --function-name "$LIFECYCLE_FN" \
  ...
  /tmp/p7-resume-out.json --no-cli-pager 2>&1 || true
if [ ! -f /tmp/p7-resume-out.json ]; then
  echo "   Lambda invoke failed (no response file)"
  exit 1
fi
RESUME_STATUS=$(jq -r '.statusCode // empty' /tmp/p7-resume-out.json 2>/dev/null)
if [ "$RESUME_STATUS" != "200" ]; then
  echo "   Expected POST resume status 200; got $RESUME_STATUS. Body: $(jq -r '.body // "{}"' /tmp/p7-resume-out.json 2>/dev/null)"
  exit 1
fi
echo "   POST resume: 200"

# 3. GET /plans/:planId/ledger — assert at least one entry (e.g. PLAN_RESUMED)
...
aws lambda invoke --region "$REGION" --function-name "$LIFECYCLE_FN" \
  ...
  /tmp/p7-ledger-out.json --no-cli-pager 2>&1 || true
if [ ! -f /tmp/p7-ledger-out.json ]; then
  echo "   Lambda invoke failed (no response file)"
  exit 1
fi
LEDGER_STATUS=$(jq -r '.statusCode // empty' /tmp/p7-ledger-out.json 2>/dev/null)
LEDGER_BODY=$(jq -r '.body // "{}"' /tmp/p7-ledger-out.json 2>/dev/null)
if [ "$LEDGER_STATUS" != "200" ]; then
  echo "   Expected GET /plans/:id/ledger status 200; got $LEDGER_STATUS. Body: $LEDGER_BODY"
  exit 1
fi
ENTRIES_COUNT=$(echo "$LEDGER_BODY" | jq -r '.entries // [] | length' 2>/dev/null || echo "0")
if [ "${ENTRIES_COUNT:-0}" -lt 1 ] 2>/dev/null; then
  echo "   Expected at least one ledger entry; got: $LEDGER_BODY"
  exit 1
fi
HAS_EVENT_TYPE=$(echo "$LEDGER_BODY" | jq -r '[.entries[]? | .event_type] | length' 2>/dev/null || echo "0")
if [ "${HAS_EVENT_TYPE:-0}" -lt 1 ] 2>/dev/null; then
  echo "   Expected ledger entries to have event_type; got: $LEDGER_BODY"
  exit 1
fi
echo "   GET /plans/:id/ledger: 200, entries count >= 1, event_type present"

echo ""
echo "Phase 7 E2E (Plan Ledger) PASSED"
