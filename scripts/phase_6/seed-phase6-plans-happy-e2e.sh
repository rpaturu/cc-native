#!/bin/bash
# Phase 6 E2E seed (Plans API happy path): write one PAUSED plan to RevenuePlans (no other ACTIVE for same tenant/account/plan_type).
# Resume should succeed (200) and plan becomes ACTIVE.
...
if [ -z "$REGION" ] || [ -z "$PLANS_TABLE" ]; then
  echo "Phase 6 E2E seed (plans happy) requires: AWS_REGION, REVENUE_PLANS_TABLE_NAME (or REVENUE_PLANS_TABLE). Set in .env or environment."
  exit 1
fi
TENANT_ID=${TENANT_ID:-e2e-p6-plans-tenant}
ACCOUNT_ID=${ACCOUNT_ID:-e2e-p6-plans-account}
PLAN_TYPE="RENEWAL_DEFENSE"
TS=$(date +%s)
PLAN_ID="e2e-p6-plans-${TS}"
PK="TENANT#${TENANT_ID}#ACCOUNT#${ACCOUNT_ID}"
NOW_ISO=$(date -u +%Y-%m-%dT%H:%M:%SZ)

echo "Phase 6 E2E seed (plans happy) — PLAN_ID=$PLAN_ID"
echo "==========================================="
...
PLAN_ITEM=$(jq -cn \
  ...
  '{
    ...
    objective: { S: "E2E Plans API happy path" },
